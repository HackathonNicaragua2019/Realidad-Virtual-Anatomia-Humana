...
public class AutoCaminar : MonoBehaviour
{
    ...
    void Update()
    {
        if (CaminarCuandoMiramos && !CaminarCuandoPulsamos && !EstaCaminando && VisionVR.transform.eulerAngles.x >= AnguloHumbral && VisionVR.transform.eulerAngles.x <= AnguloRecto)
        {
            EstaCaminando = true;
        }
        else if (CaminarCuandoMiramos && !CaminarCuandoPulsamos && EstaCaminando && (VisionVR.transform.eulerAngles.x <= AnguloHumbral || VisionVR.transform.eulerAngles.x >= AnguloRecto))
        {
            EstaCaminando = false;
        }
